...
live_loop :wobble_bass, sync: :bd, auto_cue: false do
  use_bpm 120
  sl = 0.2
  sy = ring(:tb303, :zawa).tick
  with_fx :wobble, phase: [0.2, 0.3, 0.8].choose, res: 0.6 do
    c = synth sy, note: n_cos.first, sustain: sl * n_cos.length, release: sl,
      cutoff: 100, amp: 0.7, res: 0.0
    n_cos.rotate(1).each do |note|
      sleep sl
      control c, note: note
    end
    sleep sl if sy = :tb303
  end
end

live_loop :bd do
  density knit(1,3, 2,1).tick do
    sample :bd_sone, amp: 1.8, cutoff: 100
    sleep 0.5
  end
end
